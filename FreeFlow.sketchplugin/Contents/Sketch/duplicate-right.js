  ...
  if (newLayer.parent.type === "Artboard" || newLayer.parent.type === "Group") {
      ...
      if (newLayer.parent.layers.length >> 1){
        ...
        if (newLayer.parent.layers.length >> 2){
          ...
          console.log(newLayer.parent.layers.length)
          var spacing = (newLayer.parent.layers[1].frame.x - (newLayer.parent.layers[2].frame.width + newLayer.parent.layers[2].frame.x)) || spacing;
        }

        console.log("spacing")
        console.log(spacing)
        // duplicatedLayer.frame.y = newLayer.parent.layers[0].frame.y + newLayer.parent.layers[0].frame.height + spacing;
        duplicatedLayer.frame.x = newLayer.parent.layers[1].frame.x + newLayer.parent.layers[1].frame.width + spacing;
        duplicatedLayer.frame.y = newLayer.parent.layers[1].frame.y;
