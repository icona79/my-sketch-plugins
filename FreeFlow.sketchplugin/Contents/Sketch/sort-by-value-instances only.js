...
var instructionalTextForInput = "Sort Instances by Override Values";
var descriptionText = "To sort the instances by a specific text override value, pick the value in between the ( ).\n\nTo sort in Descending order, add a 'd' after the number in the parenthesis:\n\n" + overrideLabels
...
for (j = 0; j < allSymbols.length; ++j) {
  ...
  if (allSymbols.layers[j].type === "SymbolInstance"){
      ...
      tempArray.push(allSymbols.layers[j].overrides[sortByOverrideIndex].value);
